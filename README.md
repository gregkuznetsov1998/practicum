...
* Поля: уникальный ID, имя пользователя, email, хеш пароля
* Связь: один пользователь может владеть несколькими домами

2. Дома (houses)

* Представляет физические здания/квартиры
* Поля: название, адрес, владелец (ссылка на пользователя)
* Связь: один дом содержит несколько комнат

3. Комнаты (rooms)

* Помещения внутри домов
* Поля: название, описание, ссылка на дом
* Связь: в одной комнате может быть несколько устройств
* Типы устройств (device_types)

4. Справочник типов устройств

* Поля: название, описание, единица измерения
* Связь: один тип может быть у нескольких устройств

5. Датчики (sensors)

* Обновленная сущность (ранее была sensors)
* Поля: название, тип, комната, статус, дата установки
* Связь: производит телеметрические данные

6. Телеметрические данные (telemetry_data)

* Исторические данные показаний
* Поля: значение, временная метка, качество данных
* Связь: принадлежит одному датчику
